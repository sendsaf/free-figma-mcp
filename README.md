...
When testing, enable this MCP server in one IDE first. Multiple IDEs can relay through one owner process, but a stale owner on port `3055` can make every relay look broken. See [docs/CLIENT_SETUP.md](docs/CLIENT_SETUP.md#multiple-ides) for the reset steps.
